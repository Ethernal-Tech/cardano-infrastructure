...
func TestBech32(t *testing.T) {
	tests := []struct {
		...
		{"li1dgmt3", InvalidSeparatorIndexError(2)},
		{"de1lg7wt\xff", InvalidCharacterError(0xff)},
		{"A1G7SGD8", InvalidChecksumError{"2uel5l", "g7sgd8"}},
		{"10a06t8", InvalidLengthError(7)},
		{"1qzzfhee", InvalidSeparatorIndexError(0)},
	...
	for i, test := range tests {
		...
		if !errors.Is(err, test.expectedError) {
			t.Errorf("(%d, %s): expected decoding error: `%v`. instead got `%v`",
				i, str, test.expectedError, err)
...
func TestBech32Base256(t *testing.T) {
	tests := []struct {
		...
		err:     InvalidChecksumError{"2y9e3w", "2y9e2w"},
	}, {
