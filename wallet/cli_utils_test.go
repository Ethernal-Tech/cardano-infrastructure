...
func TestDeregistrationCertificate(t *testing.T) {
	...
	stakeRegistrationCert, err := cliUtils.CreateDeregistrationCertificate(stakeAddress.String(), 3)
	require.NoError(t, err)
...
func TestGetRealEraName(t *testing.T) {
	eraName, err := NewCliUtilsForEra(ResolveCardanoCliBinary(), "latest").GetRealEraName()

	require.NoError(t, err)
	assert.Equal(t, "Babbage", eraName)

	eraName, err = NewCliUtilsForEra(ResolveCardanoCliBinary(), "conway").GetRealEraName()
