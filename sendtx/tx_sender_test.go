...
func TestCreateMetaData(t *testing.T) {
	...
	t.Run("invalid source", func(t *testing.T) {
		_, err := NewTxSender(configs).CreateMetadata(
			dummyAddr, "prime1", "vector", []BridgingTxReceiver{}, bridgingFeeAmount)
	...
	t.Run("invalid destination", func(t *testing.T) {
		_, err := NewTxSender(configs).CreateMetadata(
			dummyAddr, "prime", "vector2", []BridgingTxReceiver{}, bridgingFeeAmount)
...
func Test_getOutputAmounts(t *testing.T) {
	lovelace := getOutputAmounts([]BridgingTxReceiver{
		{
			Amount: 1,
		},
		{
	...
	assert.Equal(t, uint64(15), lovelace)
}
...
func Test_prepareBridgingTx(t *testing.T) {
	...
	txSnd := NewTxSender(map[string]ChainConfig{
		"prime": {
			MinUtxoValue:     55,
			TestNetMagic:     cardanowallet.TestNetProtocolMagic,
			TxProvider:       txProviderMock,
