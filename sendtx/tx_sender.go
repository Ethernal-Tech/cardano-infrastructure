...
func (txSnd *TxSender) CreateMetadata(
	...
	_, _, err := txSnd.getConfigs(srcChainID, dstChainID)
	if err != nil {
		return nil, err
	}
...
func (txSnd *TxSender) prepareBridgingTx(
	...
	if err := checkFee(srcConfig, txDto.BridgingFee); err != nil {
		return nil, err
	}
	...
	outputLovelaceBase := getOutputAmounts(txDto.Receivers)
...
func (txSnd *TxSender) populateTxBuilder(
	...
	potentialChangeTokenCost, err := cardanowallet.GetTokenCostSum(
		txBuilder,
		txDto.SenderAddr,
...
func adjustLovelaceOutput(
	...
	calculatedMinUtxo, err := cardanowallet.GetTokenCostSum(
...
func checkFee(config *ChainConfig, bridgingFee uint64) error {
...
func getOutputAmounts(receivers []BridgingTxReceiver) (outputCurrencyLovelace uint64) {
